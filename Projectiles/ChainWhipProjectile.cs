...
{
    public abstract class ChainWhipProjectile : ModProjectile
    {
        // The folder path to the flail chain sprite
        private const string ChainTexturePath = "StarSailor/Projectiles/ChainWhipChain";

        public override void SetStaticDefaults()
        {

            DisplayName.SetDefault("Chain Whip"); // Set the projectile name to Example Flail Ball
        }
        ...
        public override void AI()
        {
            ...
            projectile.rotation = vectorToPlayer.ToRotation() - (float)(Math.PI / 2);
    ...
    public class ChainWhipV1V2V3Projectile : ChainWhipProjectile
    {

    }
    public class ChainWhipVMaxProjectile : ChainWhipProjectile
    {
        NPC stuckTo = null;
        int stuckTimer = 0;
        bool canStick = true;
        ...
        {
            if (!canStick) damage /= 6;
            base.ModifyHitNPC(target, ref damage, ref knockback, ref crit, ref hitDirection);
        }
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            if (stuckTo == null && canStick)
            {
                stuckOldPos = target.position;
                stuckTo = target;
                canStick = false;
                stuckTimer = 90;
            }
        ...
        public override void AI()
        {
            ...
            else if (projectile.ai[0] == 1f)
            {
                if (stuckTo == null || dragging)
                {
                    ...
                    projectile.velocity *= 0.98f;
                    projectile.velocity += elasticAcceleration;
                    if (dragging) stuckTo.velocity = projectile.velocity;
                }
                else
                {

                    projectile.position += stuckTo.position - stuckOldPos;
                    stuckOldPos = stuckTo.position;
                    projectile.velocity = Vector2.Zero;
                    if (--stuckTimer <= 0 || !stuckTo.active) stuckTo = null;
